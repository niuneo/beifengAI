...
if __name__ == '__main__':
    # 1. 模拟数据创建
    category1 = np.random.randint(0, 10, [10000, 2]).astype(np.float)
    label1 = np.array([1] * 10000).reshape(-1, 1)
    data1 = np.concatenate((category1, label1), axis=1)
    category2 = np.random.randint(8, 18, [100, 2]).astype(np.float)
    ...
    plt.plot(over_sample_data.iloc[:, 0], over_sample_data.iloc[:, 1], 'ro', markersize=3)
    plt.show()
