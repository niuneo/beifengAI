...
# 3. 直接模型训练
# a. 构建随机森林模型
rf = RandomForestClassifier(n_estimators=50, max_depth=5, random_state=28)
...
# 4. 模型效果评估
y_train_pred = algo.predict(one_hot_encoder.transform(rf.apply(x_train)))
y_test_pred = algo.predict(one_hot_encoder.transform(rf.apply(x_test)))
y_pred = algo.predict(one_hot_encoder.transform(rf.apply(X)))
