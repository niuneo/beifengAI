...
def rssError(yArr, yHatArr):
    ...
    '''
    yArr = np.around(yArr, 0).astype(np.int64)
    yHatArr = np.around(yHatArr, 0).astype(np.int64)
    return np.mean(np.equal(yArr, yHatArr))
...
def abaloneTest():
    ...
    # 使用不同的核进行预测
    oldyHat01 = lwlrTest(abX[0:99], abX[0:99], abY[0:99], a)
    oldyHat1 = lwlrTest(abX[0:99], abX[0:99], abY[0:99], b)
    oldyHat10 = lwlrTest(abX[0:99], abX[0:99], abY[0:99], c)
    ...
    # 打印出不同的核预测值 与 新数据集（测试数据集）上的真实值之间的误差大小
    newyHat01 = lwlrTest(abX[100:199], abX[0:99], abY[0:99], a)
    print(("new yHat01 error Size is :", rssError(abY[0:99], newyHat01.T)))
    newyHat1 = lwlrTest(abX[100:199], abX[0:99], abY[0:99], b)
    print(("new yHat1 error Size is :", rssError(abY[0:99], newyHat1.T)))
    newyHat10 = lwlrTest(abX[100:199], abX[0:99], abY[0:99], c)
