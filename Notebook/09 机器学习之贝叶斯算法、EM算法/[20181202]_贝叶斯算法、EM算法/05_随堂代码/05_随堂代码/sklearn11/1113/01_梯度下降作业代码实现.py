...
np.random.seed(28)
n = 1000
b_values = np.random.normal(loc=-0.0, scale=10.0, size=n)
...
# plt.subplot(1, 2, 1)
# # b_values是一个数组，这里的1000相当于把这个数组中的数字分成1000等份(从最小值到最大值之间)，然后分别计算每个区间数字出现的次数，以区间作为横轴，以出现的次数作为纵轴
...
def calc_min_value_with_one_sample(b, c, max_iter=1000, tol=0.00001, alpha=0.01):
    """
    计算y=x**2+b*x+c只有一个(b,c)组合样本的时候该函数的最小值是多少
    :param b:
    :param c:
    :param max_iter: 使用梯度下降的时候，最大的迭代次数
...
def calc_min_value_with_two_sample(b1, c1, b2, c2, max_iter=1000, tol=0.00001, alpha=0.01):
    """
    计算y=x**2+b*x+c只有两个(b,c)组合样本的时候，该函数在所有组合上的最小值是多少
    :param b:
    :param c:
    :param max_iter: 使用梯度下降的时候，最大的迭代次数
    :param tol:  使用梯度下降的时候，收敛的限制条件；指的是两次迭代之间变化小于该值的时候结束参数的更新
...
def calc_min_value_with_ten_sample(b_values, c_values, max_iter=1000, tol=0.00001, alpha=0.01):
    """
    计算y=x**2+b*x+c只有十个(b,c)组合样本的时候，该函数在所有组合上的最小值是多少
    :param b_values:
    :param c_values:
    :param max_iter: 使用梯度下降的时候，最大的迭代次数
    :param tol:  使用梯度下降的时候，收敛的限制条件；指的是两次迭代之间变化小于该值的时候结束参数的更新
    ...
    :return:
    """
    # 断言：如果assert后的表达式执行为False，那么报错；否则通过判断，继续执行。
...
def calc_min_value_with_n_sample_bgd(b_values, c_values, n, max_iter=1000, tol=0.00001, alpha=0.01):
    """
    计算y=x**2+b*x+c只有n个(b,c)组合样本的时候，该函数在所有组合上的最小值是多少
    :param b_values:
    :param c_values:
    :param max_iter: 使用梯度下降的时候，最大的迭代次数
    :param tol:  使用梯度下降的时候，收敛的限制条件；指的是两次迭代之间变化小于该值的时候结束参数的更新
    ...
    def f(x, b_values, c_values):
        y = 0.0
        for b, c in zip(b_values, c_values):
            # 在这里求均值的目的：1.为了梯度更新的时候，梯度值比较小，这样x更新的时候才会收敛； 2. 防止数据量太大，溢出越界。
            y += f1(x, b, c) / n
        return y
...
def calc_min_value_with_n_sample_sgd(b_values, c_values, n, max_iter=10, tol=0.00001, alpha=0.01):
    """
    计算y=x**2+b*x+c只有n个(b,c)组合样本的时候，该函数在所有组合上的最小值是多少
    :param b_values:
    :param c_values:
...
# calc_min_value_with_two_sample(b_values[0], c_values[0], b_values[1], c_values[1])
# calc_min_value_with_ten_sample(b_values, c_values)
# calc_min_value_with_n_sample_bgd(b_values, c_values, n)
calc_min_value_with_n_sample_sgd(b_values, c_values, n)

# TODO: 自己考虑一下如果基于MBGD如何实现
