...
def read_images(parent_file_path):
    """
    从给定的文件夹中读取图像数据以及对应的标签数据，要求给定的parent_file_path下面必须是文件夹，
    文件夹下必须是图片，文件夹名称作为标签数据
    :param parent_file_path: 给定是一个文件夹路径，eg: “./中文字符识别/训练数据”或者“./中文字符识别/验证数据”
    :return:
    """
    X = None
    Y = []
...
if __name__ == '__main__':
    ...
    if is_train:
        # 1.加载数据
        x, y = read_images('./中文字符识别/训练数据')
        x_valid, y_valid = read_images('./中文字符识别/验证数据')
        ...
        pca = PCA(n_components=int(min(25, 0.9 * x_train.shape[1])), random_state=25)
        ...
        algo = SVC(kernel='rbf', C=2.0, gamma=0.1, random_state=25)
