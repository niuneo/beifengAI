# -- encoding:utf-8 --
"""
@File : 02_基于sklearn的机器学习编程流程.py
@Author: Octal_H
@Date : 2019/9/17
@Desc : 只要是机器学习领域，编程的流程基本和该文件中的内容一致
"""
...
# 2.数据清洗
# 将问号改成nan
# inplace：该参数的含义是指是否在当前对象上直接修改，默认为False，表示在新对象上修改，原对象不改变;
#             设置为True就表示直接在当前原始对象上修改
df.replace('?', np.nan, inplace=True)
# 删除为nan的数据
# axis：指定按照什么维度来删除数据，0表示第一维，也就是DataFrame中的行。1表示列
# how：指定进行什么样的操作，any表示只要出现任意一个特征属性为nan，那么就删除当前行或者当前列。
#                            all表示只有当所有的特征属性值均为nan的时候，才删除当前行货当前列
...
# 5.特征工程的操作
"""
特征工程也就是特征数据转换，直白来讲就是讲数据从A --> B
在转换的时候会基于一定的转换规则，这个转换规则你可以认为是函数，所以这个转换规则需要从训练数据中学习
NOTE：特征工程也是需要训练的，和算法模型一样
"""
"""
sklearn中所有特征工程以及算法的API基本都一样，主要API如下：
fit：基于传入的x和y进行模型的训练
transform：使用训练好的模型参数对传入的x做一个数据的转换操作，该API一般出现在特征工程中
fit_transform：这是fit和transform两个API的组合，基于传入的x和y首先做一个模型训练操作，
                然后基于训练好的模型做对x做一个转换的操作，该API一般出现在特征工程中
predict：使用训练好的模型对传入的特征属性x做一个模型预测的操作，得到对应的预测值。该API一般出现在算法模型中
score：对传入的x和y使用训练好的模型进行预测，然后将预测值和实际值进行比较得到模型的评估指标，
        在分类算法中，该API返回准确率；在回归算法中，该API返回的是R2
"""
# StandardScaler：对特征属性中的每一列都进行转换操作，将每列特征数据转化为服从均值为0，方差为1的高斯分布数据
# a.创建对象
...
normalize=False：在模型训练之前是否对数据做一个归一化的处理。默认表示不进行，该参数一般不该
copy_X=True：对于训练数据是否copy一份再训练，默认是
n_jobs=1：指定使用多少个线程来训练模型。默认为1
NOTE：处理fit_intercept之外，其他参数基本不修改
"""
