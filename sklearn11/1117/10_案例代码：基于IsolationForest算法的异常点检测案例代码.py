# -- encoding:utf-8 --
"""
@File : 10_案例代码：基于IsolationForest算法的异常点检测案例代码
@Author: Octal_H
@Date : 2019/10/15
@Desc : 
"""
...
# 模型构建
algo = IsolationForest(n_estimators=10)
algo.fit(x_train)
...
print(y_pred_test)
y_pred_outliners = algo.predict(x_outliers)
print(y_pred_outliners)
...
for algo1 in algo.estimators_:
    ...
    graph.write_png('images/isolation_{}.png'.format(k))
    k += 1
    if k > 3:
        break
