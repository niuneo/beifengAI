...
def fit(X, Y, alpha=0.0001, max_iter=None, tol=1e-5, fit_intercept=True):
    ...
    # 5.返回结果
    print("迭代{}次后，损失函数为：{}".format(k, pred_j))
...
if __name__ == '__main__':
    ...
    # c.使用自己实现的算法进行模型训练
    theta, intercept, pred_j = fit(x, y, alpha=0.01, max_iter=2000, tol=1e-16)
    ...
    plt.plot(x_test, y_predict1, color='#3568AA', lw=2, label=u'最小二乘的线性回归模型：%.3f' % s1)
    plt.plot(x_test, y_predict2, color='#b624db', lw=2, label=u'随机梯度下降的线性回归模型：%.3f' % s2)
